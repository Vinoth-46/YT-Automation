...
class ScriptEngine:
    ...
    async def generate_full_content(self, existing_topics=None, custom_topic=None):
        ...
        prompt = (
            ...
            f"   - An 'ai_video_prompt' in English containing a highly detailed, photorealistic visual prompt for a text-to-video diffusion model (e.g., Wan 2.1). Describe specific camera movements (e.g., slow panning, zoom, steady tracking shot), lighting (e.g., cinematic, sunny day, golden hour), and concrete, physically-represented construction actions. It must match the narration context 100%. (e.g. 'Cinematic tracking shot of a brick mason carefully spreading mortar and laying red clay bricks, close-up, photorealistic, 4k').\n"
            ...
            f"   - A 'narration_tamil' string containing the specific segment of the Tamil voiceover script spoken during this scene (approx 20-25 words). The concatenation of all 6 'narration_tamil' blocks must equal the overall 'narration' block.\n"
            ...
            f"  \"script\": {{\n"
            f"    \"narration\": \"Full combined Tamil script here (concatenation of all narration_tamil fields)\", \n"
            f"    \"scenes\": [\n"
            f"      {{\"visual_query\": \"...\", \"ai_video_prompt\": \"Detailed photorealistic prompt for AI generator\", \"text_overlay\": \"...\", \"narration_tamil\": \"Tamil narration spoken during this scene (approx 20-25 words)\", \"animation\": {{\"type\": \"...\", \"title\": \"...\", \"details\": {{...}}}}}},\n"
            f"      {{\"visual_query\": \"...\", \"ai_video_prompt\": \"Detailed photorealistic prompt for AI generator\", \"text_overlay\": \"...\", \"narration_tamil\": \"Tamil narration spoken during this scene (approx 20-25 words)\", \"animation\": {{\"type\": \"...\", \"title\": \"...\", \"details\": {{...}}}}}},\n"
            f"      {{\"visual_query\": \"...\", \"ai_video_prompt\": \"Detailed photorealistic prompt for AI generator\", \"text_overlay\": \"...\", \"narration_tamil\": \"Tamil narration spoken during this scene (approx 20-25 words)\", \"animation\": {{\"type\": \"...\", \"title\": \"...\", \"details\": {{...}}}}}},\n"
            f"      {{\"visual_query\": \"...\", \"ai_video_prompt\": \"Detailed photorealistic prompt for AI generator\", \"text_overlay\": \"...\", \"narration_tamil\": \"Tamil narration spoken during this scene (approx 20-25 words)\", \"animation\": {{\"type\": \"...\", \"title\": \"...\", \"details\": {{...}}}}}},\n"
            f"      {{\"visual_query\": \"...\", \"ai_video_prompt\": \"Detailed photorealistic prompt for AI generator\", \"text_overlay\": \"...\", \"narration_tamil\": \"Tamil narration spoken during this scene (approx 20-25 words)\", \"animation\": {{\"type\": \"...\", \"title\": \"...\", \"details\": {{...}}}}}},\n"
    ...
    async def generate_script(self, topic, retry_count=0):
        ...
        prompt = (
            ...
            "IMPORTANT: You MUST include an 'animation' block for scenes 2, 3, 4, and 5 (skip scene 1 hook and scene 6 CTA). Include the correct type and structured details (e.g. comparison points, concrete ratio cups, structural drawing coordinates, curing days progress, or warning descriptions). Add a 'narration_tamil' string block inside each scene representing the specific Tamil script spoken during that scene. Also generate a detailed 'ai_video_prompt' describing the photorealistic scene construction actions and camera movement for each scene."
        )
